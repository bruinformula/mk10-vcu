...
/**
 * @brief the actual procedure of triggering precharge relays (placeholder).
 */
void prechargeSequence(void) {
	// TODO: Implement your actual precharge logic here
}
...
int main(void)
{
  ...
  /* MCU Configuration--------------------------------------------------------*/
  HAL_Init();  // <-- If this faults, see SCB->BFAR in debugger

  /* Configure the system clock */
  SystemClock_Config();

  // Force an update of SystemCoreClock after the PLL config
  SystemCoreClockUpdate();  // <-- ensures SystemCoreClock != 0
  ...
  while (1)
  {
	  ...
	  // Fixed the time-check logic:
	  if ((HAL_GetTick() - millis_since_dma_read) > DMA_READ_TIMEOUT && dma_read_complete)
	  {
...
void SystemClock_Config(void)
{
  /* NOTE: This is your original clock code, which sets the PLL to 84 MHz. */
...
/*  ADC1, I2S2, SPI3, TIM3, DMA, and GPIO init code all remain unchanged below.
    Make sure these match your actual hardware pin mapping and device. */
